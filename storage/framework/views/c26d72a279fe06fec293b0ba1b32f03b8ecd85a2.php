				...
				<div class="row">
					<div class="col-xs-12">
						<div class="col-xs-5 padding0 naslov">
							<h1><?php echo e(trans('wines.title')); ?></h1>
						</div>	
							...
							<?php if (Auth::check() && Auth::user()->hasRole('admin|private|legal')): ?>
								<?php if (Auth::check() && Auth::user()->hasRole('private')): ?>
									<?php if(Auth::user()->ownsAnyVehicle()): ?>
										<a style="margin-left: 6px;" id="add-vehicle" class="default-button gumb" href="javascript:void(0)"><?php echo e(trans('wines.add')); ?></a>
									<?php endif; ?>
								<?php endif; ?>

								<?php if (Auth::check() && Auth::user()->hasRole('admin|legal')): ?>
									<button type="button" class="btn gumb" data-toggle="modal" data-target="#addWineModal" >
										<?php echo e(trans('wines.add')); ?> +
									</button>
								<?php endif; ?>
							<?php endif; ?>
						</div>
	...
	function editWine(wine_id) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				$('#wine-geographical-origin-label').val(data.geographical_origin_labels);
				...
				var vineyardId = data.vineyard_id;
				var geoOriginId = data.geographical_origin_labels;
				...
				.then(function (response) {
					...
					$.each(vineyards, function (index, vineyard) {
						// Provjeravamo da li je id trenutnog vinograda jednak vineyardId
						var isSelected = (vineyard.id == vineyardId) ? 'selected' : '';
						// Dodajemo opciju, sa isSelected ako je vinograd odgovarajući
						$('#vineyard').append('<option value="' + vineyard.id + '" ' + isSelected + '>' + vineyard.name + '</option>');
					});
				})
				.catch(function (error) {
				...
				var colorId = data.color_id; // Dobivanje ID-a odabrane boje
				var varietyId = data.variety_id; // Dobivanje ID-a odabrane sorte
				...
				.then(function (response) {
					...
					// Ažuriranje polja "wine-vintage-variety" s novim opcijama
					var options = '';
					varieties.forEach(function(variety) {
						var isSelected = (variety.id == varietyId) ? 'selected' : ''; // Provjera da li je trenutna sorta odabrana
						options += '<option value="' + variety.id + '" ' + isSelected + '>' + variety.name + '</option>';
					});
        ...
        $('#winesTable').DataTable({
            ...
            ajax: {
				...
				columns: [
						...
						{ data: '4', name: 'net_quantity_ml' },
						{ data: '5', name: 'grape_variety_harvest_specific' },
						{ data: '6', name: 'opcije' },
			],
			language: {
					search: 'Pretraga:',
...
<?php echo $__env->make('layouts.dashboard', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Projekti\wine360\resources\views/wine/index.blade.php ENDPATH**/ ?>