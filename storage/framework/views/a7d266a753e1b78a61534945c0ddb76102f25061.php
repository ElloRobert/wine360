...
<div class="home-dashboard-content">
	<div class="row vehicles-home text-left mt-15">
		...
		<div class="row justify-content-center align-items-center">
			...
			<div class="row">
				<div class="col-xs-12">
					<div class="col-xs-5 naslov ">
						<h2><?php echo e(trans('malfunction.title')); ?></h2>
					</div>
					<div class="col-xs-3">
					</div>
					<div class="col-xs-4 mt-10 text-right">
						...
						<?php if (Auth::check() && Auth::user()->hasRole('admin|private|legal|employee')): ?>
							<button type="button" class="btn gumb" id="openAddMalfunctionModal">
								<?php echo e(trans('malfunction.add')); ?> +
							</button>
						<?php endif; ?>
				</div>
				</div>
				...
				<div class="filteri d-none" id="filteri">
					<div class="row mb-3  mt-5" style="margin-top: 70px; margin-left:15px;">
						...
						<div class="col-sm-3 filter">
							<select id="vehicle-group-select" name="vehicle-group-select" class="form-control1 filterInput">
						...
						<div class="col-sm-3 filter">
							<select id="subcategoiry-select" name="subcategoiry-select" class="form-control1 filterInput">
						...
						<div class="col-sm-3 filter">
							...
							<select id="vehicles-list2" style="width: 100%" class= "form-control1 filterInput" name="vl" data-path=".vehicles-name"data-control-type="textbox" data-control-name="title-filter" data-control-action="filter">
			...
			<?php echo $__env->make('malfunction.addModal', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
			<?php echo $__env->make('malfunction.editModal', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
			<?php echo $__env->make('malfunction.confirmModal', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
			<?php echo $__env->make('malfunction.CropImageModal', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?> 
...
<script type="text/javascript">
	...
	$(function() {
		...
		$('#malfunctionTable').DataTable({
			...
			serverSide: true,
			ajax: {
				url: '<?php echo e(route('getMalfunctions')); ?>',
				type: 'GET',
				data: function (d) {
	...
	$(document).ready(function(){
		...
		$('#openAddMalfunctionModal').on('click', function() {
			// Otvaranje moda samo ako je pritisnuto dugme za Add Maintenance
			$('#addMalfunctionModal').modal('show');
		});
	...
	function editVehicleMalfunction(vehicleMalfunctionId) {
		var malfunction_id = vehicleMalfunctionId;
		$('#malfunction-details-model').modal('show');
		$.ajax({
			type: "POST",
			url: "<?php echo e(route('vehicles.malfunction.details')); ?>",
			data: { 
				id: malfunction_id,
				_token: "<?php echo e(csrf_token()); ?>",
			},
			success: function(data) {
				$('#malfunction-update-form').attr('action', '<?php echo e(route("vehicles.malfunction")); ?>/'+malfunction_id);

				$('#malfunction-update-form #malfunctionDetailsName-edit').val(data.name);
					...
					$.ajax({
						...
						success: function (data) {
							...
							$.each(data.vehicles_list, function (index, vehicle) {
								...
								console.log(isSelected, data.vehicles_list);
								$vehiclesList2.append($('<option>', {
									value: vehicle.id,
									text: vehicle.text,
									selected: isSelected 
				...
				const formattedDate = formatDate(data.date);
					$('#malfunction-update-form #malfunctionDetailsDate').val(formattedDate);
					$('#malfunction-update-form #malfunctionDetailsEndDate').val(formattedDateEnd);
					$('#malfunction-update-form #malfunctionDetails').val(data.details);
					$('#malfunction-update-form #malfunction-image-position-modal').val(data.vehicle_image_position);
					$('#malfunction-update-form #malfunctionKilometersTraveled').val(data.vehicle.kilometers);
					$('#malfunction-update-form #malfunctionKilometersTraveled').val(data.vehicle.kilometers);
					$('#malfunction-update-form #malfunctionPriority').val(data.malfunction_priority);
					$('#malfunction-update-form #malfunction-cost-neto-edit').val(data.neto_value);
					$('#malfunction-update-form #malfunction-cost-vat-edit').val(data.VAT);
					$('#malfunction-update-form #malfunction-cost-bruto-edit').val(data.bruto_value);
					$('#malfunction-update-form #malfunctionKilometersTraveled').val(data.vehicle.kilometers);
				if(data.end_date_reminder != null ){
					console.log(data.end_date_reminder != null);
					$('#malfunction-update-form #malfunction_create_reminder').val("1");
				}
	...
	function deleteVehicleMalfunction(vehicleMalfunctionId) {
		var malfunction_id = vehicleMalfunctionId;
		var malfunction_name = $('#malfunctionDetailsName').val();
		$('#confirm .modal-body .remove-malfunction-name').text(malfunction_name);
		$('#malfunction-delete-form').attr('action', '<?php echo e(route("vehicles.malfunction")); ?>/delete/'+malfunction_id);
	...
	function changeStatus(id) {
		$.ajax({
			type: 'GET',
			url: "<?php echo e(route('getMalfunction')); ?>",
			data: {
				_token: "<?php echo e(csrf_token()); ?>",
				id: id,
			},
...
<?php echo $__env->make('layouts.dashboard', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Projekti\Wine360\resources\views/malfunction/index.blade.php ENDPATH**/ ?>